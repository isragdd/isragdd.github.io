const API_URL = 'https://script.google.com/macros/s/AKfycbyavbD5f5EAh7ZT5q9y0fEGKKFof-9FtHITzKL1gvYmyi6PENSKeHPrXx8aBL1OL0E_kg/exec';
...
async function dbLoad() {
    try {
        ...
        console.warn('DB load failed, using local storage', e);
        loadFromLocalStorage();
        return false;
    }
}
...
function loadFromLocalStorage() {
    ...
    const customSaved = localStorage.getItem('customTasks');
    ...
    if (customSaved) {
        try {
            state.custom = JSON.parse(customSaved);
        } catch (e) {
...
async function dbSave() {
    ...
    try {
        ...
        console.warn('Failed to save to Google Sheets, data saved locally', e);
    }
}
...
const icons = {
    heart: (size, fill, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="${fill}" stroke="currentColor" stroke-width="2" class="${className}"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>`,
    ...
    settings: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><circle cx="12" cy="12" r="3"></circle><path d="M12 1v6m0 6v6m9.22-9.22l-4.24 4.24m-6 0L6.78 9.78M23 12h-6m-6 0H1m18.22 6.22l-4.24-4.24m-6 0L4.78 18.22"></path></svg>`,
    ...
    star: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>`,
    zap: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>`,
    shield: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>`,
    ...
    trash: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>`,
    check: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><polyline points="20 6 9 17 4 12"></polyline></svg>`,
    x: (size, className) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="${className}"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`
};
...
async function init() {
    ...
    Object.entries(db).forEach(([cat, ct]) => {
        ct.forEach(t => {
            const inc =
                t.days === 'daily' ||
                (t.days === 'weekly' && d === 0) ||
                (Array.isArray(t.days) && t.days.includes(d));
    ...
    state.custom.forEach(t => {
        ...
        if (typeof t.days === 'string' && t.days.startsWith('weekly_')) {
            inc = d === parseInt(t.days.split('_')[1]);
        } else if (Array.isArray(t.days)) {
            inc = t.days.includes(d);
        }
                        ...
                        ${state.items.map(i => {
                            ...
                            return `
                                <button onclick="buyItem('${i.id}')" ${!can ? 'disabled' : ''}
                                    class="w-full p-4 rounded flex justify-between ${can ? 'bg-purple-600-30 border-purple-500-50 hover-bg-purple-60-50' : 'bg-gray-700-30 opacity-50'}">
...
function renderPlayerView() {
    ...
    const hearts = Array(state.stats.maxHearts).fill(0).map((_, i) => 
        icons.heart(24, i < state.stats.hearts ? '#ef4444' : 'none', i < state.stats.hearts ? 'text-red-500' : 'text-gray-600')
    ).join('');
                ...
                <div class="bg-gradient-purple-blue rounded-lg p-4 border-purple-500-30">
                    ...
                    ${state.showProj ? `
                        <div class="mt-4 space-y-3">
                            <input type="text" id="projName" value="${state.projName}" oninput="state.projName=this.value" 
                                placeholder="What do you want to work on?" class="w-full bg-gray-800 rounded p-3" />
                ...
                <div class="fixed inset-0 bg-black-80 flex items-center justify-center p-4 z-50">
                    <div class="bg-gray-800 rounded p-6 max-w-sm w-full">
                        ...
                        <input type="password" id="pwInput" value="${state.pw}" 
                            ...
                            placeholder="Password" class="w-full bg-gray-700 rounded p-3 mb-4" />
                        <div class="flex gap-2">
